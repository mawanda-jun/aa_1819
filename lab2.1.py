...
# E SE AVESSIMO UN KERNEL PERSONALE?
k_lin = np.dot(X, X.T)
# non possiamo usarla tutta per il training. Dobbiamo estrarne un po' per le colonne
kernel = 'precomputed'

for train, test in skf.split(X, y):
    if kernel == 'precomputed':
        X_train = k_lin[train]  # prendiamo le righe di train
        X_train = X_train[:, train]  # prendiamo le colonne
        X_test = k_lin[train]
        X_test = k_lin[:, test]
        clf = GridSearchCV(
            svm.SVC(kernel='precomputed', gamma='auto'),
            # prende i parametri dalla griglia che gli passiamo. Se non glielo passiamo dobbiamo dirglielo noi.
